<template>
  <div class="holding-detail">
    <Header :title="holding ? `${holding.icon} ${holding.symbol}` : 'Holding Not Found'" :subtitle="holding ? holding.category : ''">
      <template #actions>
    ...
    <template v-if="holding">
      ...
      <div class="summary-card">
        <div class="summary-grid">
          <div class="summary-item">
            ...
            <div class="summary-value big">{{ formatCurrency(holding.currentValue) }}</div>
          </div>
          <div class="summary-item">
            <div class="summary-label">Cost Basis</div>
            <div class="summary-value">{{ formatCurrency(holding.costBasis) }}</div>
          </div>
          <div class="summary-item">
          ...
          <div class="summary-item">
            ...
            <div class="summary-value">{{ holding.shares }}</div>
          </div>
          <div class="summary-item">
            <div class="summary-label">Price per Share</div>
            <div class="summary-value">{{ formatCurrency(pricePerShare) }}</div>
          </div>
          <div class="summary-item">
            <div class="summary-label">Purchase Date</div>
            <div class="summary-value">{{ formatDate(holding.purchaseDate) }}</div>
          </div>
          <div class="summary-item">
      ...
      <div class="simulator-card">
        <h3 class="card-title">Sell Simulator</h3>
        <p class="card-subtitle">Estimate the tax impact of selling shares</p>

        <div class="input-group">
          <label class="input-label" for="shares-input">Shares to sell</label>
          <input
            id="shares-input"
            type="number"
            class="shares-input"
        ...
        <div class="results-grid">
          ...
          <div class="result-item">
            <div class="result-label">Cost Basis of Sold Shares</div>
      ...
      <div v-if="capitalGain > 0 && sharesToSell > 0" class="comparison-card">
        ...
        <div class="comparison-table">
          ...
          <div class="comparison-row highlight-row">
            ...
            <div class="comparison-cell">{{ (100 - (shortTermTax / proceeds) * 100).toFixed(1) }}%</div>
            <div class="comparison-cell">{{ (100 - (longTermTax / proceeds) * 100).toFixed(1) }}%</div>
          </div>
        </div>
    ...
    <div v-else class="empty-state">
      <p>Holding not found. The symbol may not exist in your portfolio.</p>
...
const holding = computed(() => {
  ...
  return portfolioStore.holdings.find(h => h.symbol === symbol) || null
})

const gain = computed(() => holding.value ? holding.value.currentValue - holding.value.costBasis : 0)
const gainPct = computed(() => holding.value && holding.value.costBasis > 0
  ? ((gain.value / holding.value.costBasis) * 100)
  : 0
)
const pricePerShare = computed(() => holding.value && holding.value.shares > 0
  ? holding.value.currentValue / holding.value.shares
  : 0
)

const taxTreatment = computed(() => holding.value ? calculateTaxTreatment(holding.value.purchaseDate) : 'short-term')
const taxBadgeLabel = computed(() => {
  if (!holding.value) return ''
  if (taxTreatment.value === 'long-term') return 'Long-term'
  const days = daysUntilLongTerm(holding.value.purchaseDate)
...
const sellRatio = computed(() => holding.value && holding.value.shares > 0
  ? sharesToSell.value / holding.value.shares
  : 0
)
const proceeds = computed(() => holding.value ? sellRatio.value * holding.value.currentValue : 0)
const costBasisSold = computed(() => holding.value ? sellRatio.value * holding.value.costBasis : 0)
...
const daysRemaining = computed(() => holding.value ? daysUntilLongTerm(holding.value.purchaseDate) : 0)
</script>
